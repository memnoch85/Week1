...
public static Integer acctNumb; 
public static List<String> dinga = new ArrayList(); 
...
//main
	...
	//Do while prompt for position (part1)
	static void promptaMon()
	{
		...
		do
		{
			...
			try {
				answere=Integer.parseInt(myScanner.nextLine());
				} catch (NumberFormatException ex) {
					System.out.println("Input is not an an integer try again!"); 				
					} 
				  
		} while(answere <1 || answere >50 );
		//System.out.println("Call positionMon");
		myScanner.close(); 
		System.out.println("The Number " + answere +  " is an Array Element in the position " + iterateMon(fiftyRando, answere));
	}
	
	
	//check position for single array (part 1) not used
	 static int iterateMon(int [] someArray, int guess) 
	{
		for(int i=0; i<=someArray.length-1; i++)
		{
			if(guess == someArray[i])
			{
			 posMon=i; 
			}
		}
	 ...
	 //printReport This data is 50 rows of 13 values per row. (part 2) not used for printing still used for checking dings
		  ...
		  for (int i =0; i < 50; i++)
			 {
				 ...
				 for (int j=0; j< 13; j++)
				 {
					 ...
					 //System.out.printf("%.0f \t", payments[i][j]);	 
				 }
				 switch(ding) {
				 case 0: scoreMon = "Good";
				 dinga.add(scoreMon);
				 break; 
				 
				 case 1: scoreMon = "Fair";
				 dinga.add(scoreMon);
				 break; 
				 
				 case 2: scoreMon = "Poor";
				 dinga.add(scoreMon);
				 break; 
				 
				 default:scoreMon = "Closed";
				 dinga.add(scoreMon);
				 break;
				 }
				 ding=0; 
				 //System.out.print(scoreMon);
				// System.out.println(); 
			 }
		 }	
	 } 
	 
	 //header for part2 I am using it for part 3. 
		 ...
		 Scanner myScanner = new Scanner(System.in); 
			do
			{
				...
				if(myScanner.hasNext())
				  try { choice=Integer.parseInt(myScanner.nextLine()); } catch
				...
				switch(choice)
				{
				case 1: System.out.println("Please enter account number:");
				try { acctNumb=(int) myScanner.nextDouble(); } catch 
				(Exception e) {
				System.out.println("Input is not an an integer try again!\n");
				break;
				}
				
				if (acctNumb >999 && acctNumb <10000)
				{ acctMon(acctNumb);
				break;}
				else 
				break;
				
				case 2: System.out.println("Customers with Missed Payments:");
				missedAccts();
				System.out.println(); 
				break; 
				
				case 3: System.out.println("Customers with Closed Accounts");
				closedAccts();
				System.out.println(); 
				break; 
	...
	public static void acctMon(Integer acctNum)
	{
		...
		int row =0; 
		 for (int i =0; i < 50; i++)
		 {
			 for (int j=0; j< 13; j++)
			 {
					 if(payments[i][j]==acctNum)
					 {
						 countExists=true;
						 System.out.println("Account element is: " +i +  " " + j);
						 row=i; 
						 System.out.println("Account information..."); 
						 System.out.println(printHeader());
						 System.out.printf("%-20s",  customers[row] +" ");
					
						 for(int k=0; k <13; k++)
						 {
							 System.out.printf("%.0f \t", payments[row][k]);
						 }
						 System.out.print(dinga.get(row));
					 }
			 	}
		 }
		 if (!countExists)
	...
	public static void missedAccts()
	{
		List<Integer> reportMon = new ArrayList();  
		
		for(int i =0; i<dinga.size(); i++)
		{
			if(!dinga.get(i).contains("Good"))
			{
				reportMon.add (i); 
			}
		}
		for(Integer el: reportMon)
		{
			System.out.println(customers[el]); 
		}
	}
	
	
	public static void closedAccts()
	{
		List<Integer> closeMon = new ArrayList();  
		
		for(int i =0; i<dinga.size(); i++)
		{
			if(dinga.get(i).contains("Closed"))
			{
				closeMon.add (i); 
			}
		}
		for(Integer el: closeMon)
		{
			System.out.println(customers[el]); 
		}
	}
